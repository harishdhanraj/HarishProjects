...
spark = SparkSession.builder.master("local[2]").appName(
    "WD30 Spark Core Prog learning").enableHiveSupport().getOrCreate()
...
df1 = spark.createDataFrame(schema_rdd2,
                            collist)  # this createDF will convert the schemardd --> row_rdd (internally) --> DF
# or
...
rdd1 = sc.textFile("file:///home/hduser/sample.dat").map(lambda x: x.split(","))
row_rdd1 = rdd1.map(lambda cols: Row(id=int(cols[0]), name=cols[1], sal=int(
    cols[2])))  # if we reprepresent data from notepad to xls (colname and dtype)
...
# 5. write sql queries
rdd1 = sc.textFile("file:///home/hduser/mrdata/courses.log").flatMap(lambda x: x.split(" ")).map(
    lambda x: (x,))  # functional transformation
collst = ["coursename"]
...
delfile_df1 = spark.read.csv(
    "file:///home/hduser/sparkdata/nyse.csv")  # this will create df with default comma delimiter
delfile_df2 = spark.read.option("delimiter", "~").csv(
    "file:///home/hduser/sparkdata/nyse.csv")  # this will create df with custom ~ delimiter
...
delfile_df3 = spark.read.option("delimiter", "~").option("inferschema", "true").csv(
    "file:///home/hduser/sparkdata/nyse.csv")  # this will create df with custom ~ delimiter and automatically infer schema (dataype)
...
delfile_df6 = spark.read.format("csv").options(delimiter="~", inferschema=True).load(
...
delfile_df7 = spark.read.csv("file:///home/hduser/sparkdata/nyse.csv", sep='~', inferSchema=True).toDF("exchange",
                                                                                                       "stock", "value")
...
delfile_df8_1_header = spark.read.option("header", "false").option("inferschema", "true").option("delimiter", "~").csv(
...
delfile_df8_header = spark.read.option("header", "true").option("inferschema", "true").option("delimiter", "~").csv(
    "file:///home/hduser/sparkdata/nyse_header.csv")
delfile_df8_header.printSchema()  # multiple columns
# or
# preference 1 if regular options are going to be used
delfile_df9_header = spark.read.options(header="true", inferschema="true", delimiter="~").csv(
    "file:///home/hduser/sparkdata/nyse_header.csv")
delfile_df9_header.printSchema()  # multiple columns
# or
# preference 1 if additional options are going to be used
delfile_df10_header = spark.read.csv("file:///home/hduser/sparkdata/nyse_header.csv", header="true", inferSchema="true",
                                     sep="~")
delfile_df10_header.printSchema()  # multiple columns
# or
# preference 3 if other sources we are going to use
delfile_df11_header = spark.read.format("csv").options(header="true", inferschema="true", delimiter="~").load(
    "file:///home/hduser/sparkdata/nyse_header.csv")
...
# NYSE~CVH~24.62
# NYSE~CVL~30.2
delfile_df12_header = spark.read.options(header="true", inferschema="true", delimiter="~").csv(
    "file:///home/hduser/sparkdata/nyse_header.csv")
...
# withColumnRenamed
delfile_df13_header = spark.read.options(header="true", inferschema="true", delimiter="~").csv(
    "file:///home/hduser/sparkdata/nyse_header.csv")
...
delfile_df13_2_withColumnRenamed = delfile_df13_header.withColumnRenamed("exchange_name", "exch").withColumnRenamed(
    "closing_rate", "close_rate")  # to rename one
...
delfile_df13_header = spark.read.options(header="true", inferschema="true", delimiter="~").csv(
    "file:///home/hduser/sparkdata/nyse_header.csv")
delfile_df13_1_alias = delfile_df13_header.select(col("exchange_name").alias("exch"), "stock_name",
                                                  col("closing_rate").alias(
                                                      "close_rate"))  # DSL #to rename one or more columns
...
delfile_df13_2_alias = spark.sql(
    "select exchange_name exch,stock_name,closing_rate as close_rate from view1_header")  # familiar sql
...
delfile_df13_header = spark.read.options(header="true", inferschema="true", delimiter="~").csv(
    "file:///home/hduser/sparkdata/nyse_header.csv")
aggr_df14_1_alias = delfile_df13_header.groupby("exchange_name").agg(
    min(col("closing_rate")).alias("min_close_rate"))  # dsl way
...
aggr_df14_1_alias.write.saveAsTable(
    "hive_tbl11")  # alias is mandatory for the column name definition as per hive table columname standard
...
df_to_hive_json_orc = spark.sql(
    "select exchange_name,min(closing_rate) as min_close_rate from view1 group by exchange_name")  # ISO/ANSI SQL
...
df14_noheader_inferschema = spark.read.options(delimiter="~", inferschema=True).csv(
    "file:///home/hduser/sparkdata/nyse.csv").toDF("exch_name", "stock_name", "closed_rate")
...
df14_noheader_inferschema1 = spark.read.options(delimiter="~", inferschema=True).csv(
    "file:///home/hduser/sparkdata/nyse_vinoth.csv").toDF("exch_name", "stock_name", "closed_rate")
...
strct = StructType([StructField("exch_name", StringType(), False), StructField("stock_name", StringType()),
                    StructField("close_rate", FloatType())])
df14_noheader_customschema = spark.read.schema(strct).options(delimiter="~").csv(
    "file:///home/hduser/sparkdata/nyse.csv")
...
strct = StructType([StructField("exch_name", StringType(), False), StructField("stock_name", StringType(), True),
                    StructField("close_rate", FloatType(), True)])
df15_header_customschema = spark.read.schema(strct).options(header=True, delimiter="~").csv(
    "file:///home/hduser/sparkdata/nyse_header.csv")
...
df16_choose_minimal_columns = spark.read.csv("file:///home/hduser/hive/data/txns").select(
    col("_c0").alias("txnno").cast("int"), col("_c3").alias("amt").cast("float"), col("_c6").alias("state"))
...
df17_choose_minimal_columns = spark.read.csv("file:///home/hduser/hive/data/txns").select(col("_c0").cast('int'),
                                                                                          col("_c3").cast("float"),
                                                                                          "_c6")
...
strct1 = StructType([StructField("txnno", IntegerType(), False), StructField("amount", FloatType(), True),
...
lookup_enriched_df = spark.sql(
    "select d.*,upper(f.state) as state from dbview d left join fileview f on d.city=f.city where d.city is not null")
...
hive_summary_df = dbdf1.groupby("city", "age").agg(max("createdt").alias("max_create_dt"),
                                                   sum("transactamt").alias("sum_trans_amt"))
hive_summary_df.show(2)
...
hive_summary_df.write.mode("overwrite").saveAsTable(
    "default.hive_cust_summary")  # if the table present, it will either allow me to append or overwrite

# 2. Writing Tempview data into Hive table (SQL)
hive_summary_df.createOrReplaceTempView("hive_db_view")
# spark.sql("create table default.hive_cust_summary1 as select * from hive_db_view")
spark.sql(
    "insert into table default.hive_cust_summary select * from hive_db_view")  # If we need to write data into hive using SQL
# or
spark.sql("""insert overwrite table default.hive_cust_summary select * from 
...
query = "(select custid,age,city,createdt,transactamt,upddt from customer_upd where upddt='2023-04-06')t"
dfdb1 = spark.read.jdbc(url="jdbc:mysql://localhost:3306/custdb?user=root&password=Root123$", table=query,
...
# spark.sql("insert overwrite table hive_cust_summary_ext partition(upddt) select * from dbqueryview")
# hive_summary_df.write.mode("overwrite").saveAsTable("default.hive_cust_summary")
...
dfhive1 = spark.read.table("default.hive_cust_summary")
...
df_csv1 = spark.read.option("header", "True").option("delimiter", "~").option("inferschema", "true").csv(
    "file:///home/hduser/sparkdata/nyse.csv")
...
df_transformed = df_csv1.withColumn("stock", lower(col("stock"))).withColumn("numeric_closerate",
                                                                             col("closerate").cast("int")).withColumn(
    "curdt", current_date())
...
spark.sql(
    "create table hive_curated_table_for_DS_part_hive (exchange string,closerate decimal(10,2),numeric_closerate int,curdt date) partitioned by (stock string)")
